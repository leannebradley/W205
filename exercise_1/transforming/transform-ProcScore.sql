...
CREATE TABLE ProcedureScore AS
	SELECT effective_care.MeasureID
		...
		, (Avg(case when effective_care.Score = 'Not Available' then null else cast(effective_care.Score as decimal(10,2)) end) - MeasureSummary.AvgScore)/MeasureSummary.StDevScore as RelativeScore
	FROM effective_care
...
INSERT INTO TABLE ProcedureScore 
	select effective_care.MeasureID
		...
		, (MeasureSummary.AvgScore - Avg(case when effective_care.Score = 'Not Available' then null else cast(effective_care.Score as decimal(10,2)) end))/MeasureSummary.StDevScore as RelativeScore
	FROM effective_care
	join MeasureSummary on (MeasureSummary.MeasureID = effective_care.MeasureID)
	group by effective_care.MeasureID, effective_care.MeasureName, effective_care.HospitalName;